...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/provider/checkProCodeExists",//请求的url
		data:{method:"ucexist",proCode:proCode.val()},//请求参数
		dataType:"json",//ajax接口（请求url）返回的数据类型
		success:function(data){//data：返回数据（json对象）
			// alert(data);
			if(data == "exists"){//已存在，错误提示
				validateTip(userCode.next(),{"color":"red"},imgNo+ " 该供应商编码已存在",false);
			}else{//可用，正确提示
				validateTip(userCode.next(),{"color":"green"},imgYes+" 该供应商编码可以使用",true);
			}
		},
